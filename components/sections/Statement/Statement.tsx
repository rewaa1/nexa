...
const formatStat = (currentValue: number, stat: Stat) =>
  (stat.decimals
    ? currentValue.toFixed(stat.decimals)
    : Math.round(currentValue).toString()) + stat.suffix;
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        gsap.to(counter1, {
          ...
          onUpdate: () => {
            if (stat1Ref.current)
              stat1Ref.current.textContent = Math.round(counter1.value) + "+";
          },
        });
        ...
        gsap.to(counter2, {
          ...
          onUpdate: () => {
            if (stat2Ref.current)
              stat2Ref.current.textContent = Math.round(counter2.value) + " yr";
          },
        });
        ...
        gsap.to(counter3, {
          ...
          onUpdate: () => {
            if (stat3Ref.current)
              stat3Ref.current.textContent = counter3.value.toFixed(1) + "★";
          },
        });
      },
    });
            ...
            {STATS.map((stat, index) => (
              <div key={stat.label}>
                <div className="font-display text-[28px] font-bold">
                  <span ref={statRefList[index]}>{formatStat(0, stat)}</span>
                </div>
                <div
