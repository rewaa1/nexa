...
export default function OrbitMap() {
  ...
  const jumpTo = (index: number) => {
    ...
    const rest = restRef.current[index];
    if (!scrollTrigger || rest == null) return;
    window.scrollTo({ top: scrollTrigger.start + rest * (scrollTrigger.end - scrollTrigger.start), behavior: "smooth" });
  };
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      stRef.current = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          const activeIndex = Math.round(self.progress * steps);
          setActive((current) => (current === activeIndex ? current : activeIndex));
        },
      });
    ...
    <div ref={wrap} style={{ position: "relative", height: `${NODES.length * 120}vh`, background: "var(--bg)" }}>
      <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden", color: "var(--fg)" }}>
        ...
        <div ref={universe} style={{ position: "absolute", top: 0, left: 0, width: UNI.w, height: UNI.h }}>
          <svg width={UNI.w} height={UNI.h} style={{ position: "absolute", inset: 0 }}>
            ...
            {STARS.map((star, index) => (
              <circle key={index} cx={star.x} cy={star.y} r={star.r} fill="var(--fg)" opacity={0.18} />
            ))}
            {/* orbit rings */}
            {POINTS.map((point, index) => (
              <circle key={index} cx={UNI.cx} cy={UNI.cy} r={point.r} fill="none" stroke="var(--border)" strokeWidth={1} />
            ))}
            ...
            {POINTS.map((point, index) => (
              <g key={index}>
                <circle cx={point.x} cy={point.y} r={active === index ? 14 : 9} fill={active === index ? "var(--accent)" : "var(--muted)"} />
                <circle cx={point.x} cy={point.y} r={active === index ? 26 : 0} fill="none" stroke="var(--accent)" strokeWidth={1} opacity={0.5} />
              </g>
            ))}
          </svg>
        </div>
        ...
        {NODES.map((node, index) => (
          ...
          >
            <span className="eyebrow" style={{ color: "var(--accent)" }}>{node.kicker}</span>
            <h2
            ...
            <p style={{ color: "var(--muted)", fontSize: "1.05rem", maxWidth: 460 }}>{node.copy}</p>
            {node.cta && (
              <a
        ...
        <div style={{ position: "fixed", right: "clamp(1.2rem,3vw,2.4rem)", top: "50%", transform: "translateY(-50%)", zIndex: 100, display: "flex", flexDirection: "column", gap: "0.9rem", alignItems: "flex-end" }}>
          {NODES.map((node, index) => (
            <button key={index} onClick={() => jumpTo(index)} style={{ display: "flex", alignItems: "center", gap: "0.6rem", background: "none", border: "none", cursor: "pointer", color: index === active ? "var(--fg)" : "var(--muted)", padding: 0 }}>
              <span className="eyebrow" style={{ opacity: index === active ? 1 : 0, transition: "opacity 0.3s" }}>{node.nav}</span>
              <span style={{ width: index === active ? 26 : 14, height: 2, background: index === active ? "var(--accent)" : "var(--border)", transition: "all 0.35s" }} />
            </button>
          ))}
        </div>
      </div>
